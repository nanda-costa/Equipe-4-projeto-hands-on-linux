...
static int  usb_send_cmd(char *cmd, int param);                                    // Envia um comando e espera a resposta do dispositivo
...
static int usb_send_cmd(char *cmd, int param) {
    ...
    snprintf(usb_out_buffer, usb_max_size, "%s %d\n", cmd, param); // Formata o comando no buffer de saída
    ...
    while (retries > 0 && attempts > 0) {
        ...
        // Acumula os bytes recebidos em recv_line até encontrar uma linha completa ('\n')
        for (i = 0; i < actual_size; i++) {
            ...
            if (c == '\n' || recv_size >= MAX_RECV_LINE - 1) {
                ...
                if (resp_pos) {
                    resp_pos += strlen(resp_expected);
                    if (sscanf(resp_pos, "%ld", &resp_number) == 1) {
...
static ssize_t attr_show(struct kobject *sys_obj, struct kobj_attribute *attr, char *buff) {
    ...
    if (strcmp(attr_name, "led") == 0) {
        ...
        value = usb_send_cmd("GET_THRESHOLD", 0);
    } else {
        value = -1;
    }
